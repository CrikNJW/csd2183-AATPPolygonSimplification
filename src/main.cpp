...
namespace
{
    ...
    public:
        ...
        void push_back(double x, double y, int vid)
        {
            ...
            if (vid >= 0 && vid + 1 > next_generated_id)
                next_generated_id = vid + 1;
            if (!head)
            {
                head = v;
                v->prev = v;
    ...
    bool segments_intersect_nontrivial(Vec2 p1, Vec2 p2, Vec2 p3, Vec2 p4, bool ignore_shared_endpoints)
    {
        ...
        if (collinear_overlap_nontrivial(p1, p2, p3, p4))
            return true;
    ...
    double poly_area(const std::vector<Vec2> &pts)
    {
        ...
        for (size_t i = 0, n = pts.size(); i < n; ++i)
        {
            ...
            acc += static_cast<long double>(a.x) * static_cast<long double>(b.y) - static_cast<long double>(b.x) * static_cast<long double>(a.y);
        }
    ...
    struct Candidate
    {
        ...
        bool operator>(const Candidate &o) const
        {
            ...
            if (std::fabs(diff) > eps)
                return diff > 0.0;
            if (ring_id != o.ring_id)
                return ring_id > o.ring_id;
            auto id = [](Vertex *v)
            { return v ? v->original_id : -1; };
...
int main(int argc, char **argv)
{
    ...
    for (const auto &r : rings)
        input_area += signed_area_ring_input(r.points);
